...
//TODO IMPLEMENTATION OF BUTTONS
Gui::Gui(irr::gui::IGUIEnvironment* p_guienv)
{
	...
	irr::gui::IGUIButton* tower = m_guienv->addButton(irr::core::rect<irr::s32>(midPositionSidebar-40,10,midPositionSidebar+40,40),sideBar,EnumEvent::BUTTON_TOWER,L"Tower");
	tower->setImage(m_guienv->getVideoDriver()->getTexture("resources/textures/button.png"));
	tower->setScaleImage(true);

	irr::gui::IGUIButton* sell = m_guienv->addButton(irr::core::rect<irr::s32>(midPositionSidebar-40,50,midPositionSidebar+40,80),sideBar,EnumEvent::BUTTON_SELL,L"Sell");
	sell->setImage(m_guienv->getVideoDriver()->getTexture("resources/textures/button.png"));
	sell->setScaleImage(true);

	irr::gui::IGUIButton* controls = m_guienv->addButton(irr::core::rect<irr::s32>(midPositionSidebar-40,90,midPositionSidebar+40,120),sideBar,EnumEvent::BUTTON_CONTROLS,L"Controls");
	controls->setImage(m_guienv->getVideoDriver()->getTexture("resources/textures/button.png"));
	controls->setScaleImage(true);

	irr::gui::IGUIButton* startGame = m_guienv->addButton(irr::core::rect<irr::s32>(midPositionSidebar-40,130,midPositionSidebar+40,160),sideBar,EnumEvent::BUTTON_STARTGAME,L"Start game");
...
void Gui::UpdateMenu()
{
	...
	{			
		...
		irr::gui::IGUIButton* returnButton = m_guienv->addButton(irr::core::rect<irr::s32>(35,65,140,80),m_menu, EnumEvent::BUTTON_MENU,L"Return", L"Returns to the game");
		returnButton->setImage(m_guienv->getVideoDriver()->getTexture("resources/textures/button.png"));
		returnButton->setScaleImage(true);

		irr::gui::IGUIButton* quitButton = m_guienv->addButton(irr::core::rect<irr::s32>(35,95,140,110),m_menu, EnumEvent::BUTTON_QUIT,L"Quit", L"Exits program");
...
void Gui::UpdateControlsMenu()
{
	...
	if (!m_controlsMenuIsActive)
	{
		...
		irr::gui::IGUIButton* returnButton = m_guienv->addButton(irr::core::rect<irr::s32>(35,65,140,80),m_controls, EnumEvent::BUTTON_CONTROLS,L"Return", L"Returns to the game");
		returnButton->setImage(m_guienv->getVideoDriver()->getTexture("resources/textures/button.png"));
		returnButton->setScaleImage(true);
